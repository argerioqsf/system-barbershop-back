...
import { makePayBalanceUseCase } from '@/modules/finance/infra/factories/make-pay-balance'
...
export const PayBalanceTransactionController = async (
  ...
  const receiptUrl = request.file
    ...
    : undefined
  const useCase = makePayBalanceUseCase()
  const { transactions } = await useCase.execute({
    actorId: user.sub,
    unitId: user.unitId,
    affectedUserId: data.affectedUserId,
    description: data.description,
    amount: data.amount ? Money.from(data.amount) : undefined,
