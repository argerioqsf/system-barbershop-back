...
export class PrismaUnitSegmentRepository implements UnitSegmentRepository {
  async createMany(unitId: string, segmentsIds?: string[]): Promise<Prisma.BatchPayload> {
    const unitSegment = await prisma.unitSegment.createMany({
      data: segmentsIds ? segmentsIds.map((segmentId: string) => ({
        unitId,
        segmentId
      })) : []
    })
  ...
  async create(data: Prisma.UnitSegmentUncheckedCreateInput): Promise<UnitSegment> {
